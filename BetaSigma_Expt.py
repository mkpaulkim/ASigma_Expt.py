...
ent_lam1nh = tkp.ParamEntry(fp, (500, 240, 10), 500, 'lam_1nh, um')
ent_nh = tkp.ParamEntry(fp, (500, 270, 10), 5, 'Nh')
ent_gamma = tkp.ParamEntry(fp, (500, 300, 10), 0, 'gamma', rw='r')
...
def get_qns():
    ...
    lam1nh = ent_lam1nh.get_val(float)
    # gamma = ent_gamma.get_val(float)
    nh = ent_nh.get_val()
    gamma = (lam1nh / lam12)**(1/(nh - 2))
    theta1 = psi + 2. * (q1 - q0)
    ...
    for n in range(2, nh+1):
        ...
        lam_1ns += [lam1n]
        q_ns += [qn]
    ent_gamma.set_entry(f'{gamma:.3f}')

    # print(gf.prn_list('q_ns', q_ns, 4) + f' q_range = {q_ns[-1] - q_ns[1]: .4f}')
    # print(gf.prn_list('lam_ns', lam_ns, 8))
...
def make_notes():
    ...
    qq_notes = f'> psi = {ent_psi.get_val(float)}; ' \
               ...
               f'Nh = {ent_nh.get_val()}; ' \
               f'lam_1N = {ent_lam1nh.get_val(float)}; \n'
